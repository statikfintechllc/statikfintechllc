 ...
 * Generated: 2025-10-18T20:26:24.794Z
 */
...
class SFTiMobileNavbar extends BaseNavbar {
        ...
        return `
            <div class="fixed top-0 left-0 right-0 z-50">
                ...
                <nav class="bg-black/95 backdrop-blur-xl border-b border-white/10">
                    ...
                    <div id="mobile-menu" class="fixed left-0 right-0 bg-black/95 backdrop-blur-xl transform transition-transform duration-300 ease-in-out" style="top: 0; bottom: 0; z-index: 40;">
    ...
    attachEventListeners() {
        // Ensure menu is hidden on init and positioned correctly
        const mobileMenu = document.getElementById('mobile-menu');
        const navbarContainer = mobileMenu?.closest('.fixed');
        
        if (mobileMenu && navbarContainer) {
            // Calculate the correct top position (navbar + ticker height)
...
(function registerSFTiDomainBindings() {
    ...
    if (globalObj) {
        if (!globalObj['ServerNavbarMobile']) {
            globalObj['ServerNavbarMobile'] = DomainImplementation;
        }
        if (!globalObj['createServerNavbarMobile']) {
            globalObj['createServerNavbarMobile'] = (config = {}) => new DomainImplementation(config);
        }
